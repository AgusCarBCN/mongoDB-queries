...
//5.Escriu una consulta per mostrar tot els restaurants que estan en el Bronx
 ...
 db.restaurants.find({borough:"Bronx"},{_id:0,restaurant_id:1,name:1,borough:1,address:{zipcode:1}}).pretty()
 
//6.Escriu una consulta per mostrar els primers 5 restaurants que estan en el Bronx

db.restaurants.find({borough:"Bronx"},{_id:0,restaurant_id:1,name:1,borough:1,address:{zipcode:1}}).pretty().limit(5)

//7.Escriu una consulta per mostrar el pròxim 5 restaurants després de saltar els primers 5 del Bronx

db.restaurants.find({borough:"Bronx"},{_id:0,restaurant_id:1,name:1,borough:1,address:{zipcode:1}}).pretty().skip(5).limit(5)

//8.Escriu una consulta per trobar els restaurants que tenen un score de més de 90

db.restaurants.find({"grades.score":{$gt:90}},{name:1,borough:1,grades:1}).pretty()

//9.Escriu una consulta per trobar els restaurants que tenen un score de més que 80 però menys que 100
 
db.restaurants.find({grades:{$elemMatch:{"score":{$gt : 80 , $lt :100}}}},{name:1,borough:1,grades:1}).pretty();

//10.Escriu una consulta per trobar els restaurants quins localitzen en valor de latitud menys que -95.754168

db.restaurants.find({"address.coord" : {$lt : -95.754168}},{name:1,borough:1,address:{coord:1}}).pretty();
...
//11.Escriu una consulta de MongoDB per a trobar els restaurants que no preparen cap cuisine de 'American' i el seu puntaje de qualificació superior a 70 i latitud inferior a -65.754168
 ...
 db.restaurants.find({$and:[{"grades.score" : {$gt : 70}},{cuisine:{$ne:'American'}},{"address.coord":{$lt:-65.754168}}]},{name:1,borough:1,cuisine:1,"address.coord":1}).pretty();
...
db.restaurants.find({"grades.score" : {$gt : 70},cuisine:{$ne:'American'},"address.coord":{$lt:-65.754168}},{name:1,borough:1,cuisine:1,"address.coord":1}).pretty();
...
//13.Escriu una consulta per trobar els restaurants quins no preparen cap cuisine de 'American ' i va aconseguir un punt de grau 'A' no pertany a Brooklyn. S'ha de mostrar el document segons la cuisine en ordre descendent	
 ...
 db.restaurants.find({"grades.grade" :'A',cuisine:{$ne:'American'},borough:{$ne:'Brooklyn'}},{name:1,borough:1,cuisine:1,grades:{grade:1}}).sort({cuisine:-1}).pretty()
...
db.restaurants.find({name:{$in:[/^Wil/]}},{restaurant_id:1,name:1,borough:1,cuisine:1}).pretty()
...
db.restaurants.find({name:{$in:[/ces$/]}},{restaurant_id:1,name:1,borough:1,cuisine:1}).pretty()
...
//16.Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants quin contenir 'Reg' com tres lletres en algun lloc en el seu nom
 ...
 db.restaurants.find({name:{$in:[/.*Reg.*/]}},{restaurant_id:1,name:1,borough:1,cuisine:1}).pretty()
 
//17.Escriu una consulta per trobar els restaurants quins pertanyen al Bronx i va preparar qualsevol plat American o xinès

 db.restaurants.find({borough:'Bronx',$or:[{cuisine:'American'},{cuisine:'Chinese'}]},{name:1,cuisine:1,borough:1}).pretty()
...
//20.Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants que aconsegueixin un marcador quin no és més que 10 (¿que no es mes que 10?)
 
db.restaurants.find({$nor:[{"grades.scores":{$lt:10}}]},{restaurant_id:1,name:1,borough:1,cuisine:1}).pretty() 

//21.Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants que preparen peix excepte 'American' i 'Chinees' o el name del restaurant comença amb lletres 'Wil'.....No se a que es refereix la pregunta....que preparen peix?.
...
db.restaurants.find({"grades.grade":"A","grades.date": ISODate("2014-08-11T00:00:00Z"),"grades.score":11},{restaurant_id:1,name:1,grades:1}).pretty()

//23.Escriu una consulta per trobar el restaurant_id, name i grades per a aquells restaurants on el 2n element de varietat de graus conté un grau de "A" i marcador 9 sobre un ISODate "2014-08-11T00:00:00Z". El segon element de l'array es el 1.

db.restaurants.find({"grades.1.grade":"A","grades.1.date": ISODate("2014-08-11T00:00:00Z"),"grades.1.score":9},{restaurant_id:1,name:1,grades:1}).pretty()
...
db.restaurants.find({"address.coord.1":{$gt:20},"address.coord.1":{$lt:52}},{restaurant_id:1,name:1,address:1,address:{coord:1}}).pretty()
...
db.restaurants.find({},{name:1,cuisine:1}).sort({cuisine:1},{borough:-1}).pretty()

//28.Escriu una consulta per saber tant si totes les direccions contenen el carrer o no

db.restaurants.find({},{address:{street:1}}).pretty()
...
db.restaurants.find({name:{$in:[/.*mon.*/]}},{name:1,borough:1,"address.coord":1}).pretty()
...
//32.Escriu una consulta per trobar el name de restaurant, borough, longitud i latitud i cuisine per a aquells restaurants que conteinen 'Mad' com primeres tres lletres del seu name
 ...
 db.restaurants.find({name:{$in:[/^Mad/]}},{name:1,borough:1,"address.coord":1,cuisine:1}).pretty()
